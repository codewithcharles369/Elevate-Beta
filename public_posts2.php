...
if (isset($_GET['category_id'])) {
    $filter_sql = "WHERE posts.category_id = ?";
...
// Query to fetch posts along with user name, category, and like status
$stmt = $pdo->prepare("
    SELECT 
        posts.id,
        posts.title,
    ...
    $filter_sql AND  scheduled_at IS NULL OR scheduled_at <= NOW() AND 
    audience = 'public' 
...
");
array_unshift($params, $_SESSION['user_id'], $user_id, $user_id, $user_id); // Add the logged-in user ID as the first parameter
$stmt->execute($params);
...
if (empty($filter_sql)) {
    ...
    FROM posts
        ...
        ORDER BY posts.created_at DESC
    ");
    $stmt->execute([$_SESSION["user_id"], $user_id, $user_id, $user_id]);
    $posts = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
?>
...
<?php endif; ?>
        ...
        <div class="blog-content">
            ...
            <div class='post-meta'>
                <span><i class='fas fa-user'></i> By <?php echo htmlspecialchars($post['name']); ?></span>
