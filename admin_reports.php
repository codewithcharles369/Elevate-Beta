...
$stmt = $pdo->query("SELECT reports.id, reports.status,  reports.reason, reports.user_id, posts.title AS post_title, posts.id AS post_id, users.name AS reporter 
                     FROM reports 
                     JOIN posts ON reports.post_id = posts.id 
                     JOIN users ON reports.user_id = users.id ORDER BY reports.created_at DESC");
...
<body class="<?php echo htmlspecialchars($theme); ?>">
    ...
    <aside class="sidebar" style="overflow-y: scroll;">
        ...
        <ul class="nav">
              ...
              <a href=""><li  class="animate-on-scroll icon"><a href="admin_reports.php"><i class="fas fa-chart-line"> <?php if ($report_count > 0): ?><span class="count-badge"><?= $report_count ?></span><?php endif; ?></i></a></li>
    ...
    <input type="search" id="myInput" onkeyup="myFunction()" class="search" placeholder="Search for Report.." title="Type in a report">
    <table id="myTable" class="user-table">
      ...
      <?php foreach ($reports as $report): ?>
            <tr class="animate-on-scroll">
                <td class="animate-on-scroll"><?php echo htmlspecialchars($report['reason']); ?></td>
                <td class="animate-on-scroll"><?php echo htmlspecialchars($report['post_title']); ?></td>
                <td class="animate-on-scroll center-text">
                <a style=" text-decoration: none;" href="user_profile.php?id=<?php echo $report['user_id']; ?>">
                    <?php echo htmlspecialchars($report['reporter']); ?></td>
                    </a>
                <td class="animate-on-scroll center-text"><?php echo htmlspecialchars($report['status']); ?></td>
                <td class="animate-on-scroll">
                    <a class="btn" href="view_post2.php?id=<?php echo $report['post_id']; ?>">View Post</a>
                    <?php if ($report['status'] === 'unresolved'): ?>
                        <a class="btn delete-btn" href="resolve_report.php?id=<?php echo $report['id']; ?>">Resolve</a>
                    <?php endif; ?>
                </td>
            </tr>
        <?php endforeach; ?>
