...
$stmt = $pdo->prepare("
    SELECT 
        ...
        (SELECT COUNT(*) FROM likes WHERE post_id = posts.id AND created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)) AS like_count,
        (SELECT COUNT(*) FROM comments WHERE post_id = posts.id AND created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)) AS comment_count,
        (posts.views + 
        (2 * (SELECT COUNT(*) FROM likes WHERE post_id = posts.id AND created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY))) + 
        (3 * (SELECT COUNT(*) FROM comments WHERE post_id = posts.id AND created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)))) AS trending_score
    FROM posts
...
<body class="<?php echo htmlspecialchars($theme); ?> trending-posts">
      <!-- Sidebar -->
    ...
    <aside style="height: 100%;  overflow-y: scroll" class="sidebar">
        ...
        ?>
            <a class="btn" style="background-color: #dc3545;" href="public_posts.php?category_id=<?php echo $category['id']; ?>"><?php echo htmlspecialchars($category['name']); ?></a>
        <?php endforeach; ?>
        ...
        foreach ($tags as $tag):
        ?>
            <a class="btn" href="public_posts.php?tag_id=<?php echo $tag['id']; ?>"><?php echo htmlspecialchars($tag['name']); ?></a>
        <?php endforeach; ?>
            ...
            <ul class="blog-cards">
                ...
                <li class="blog-card">
                    ...
                    <?php if (!empty($post['media'])): ?>
                        ...
                        <?php endif; ?>
                    <?php endif; ?>
                        <small>  <!--<button class=" trends-btn"><?php echo $post['views']; ?> View(s)</button> <button class="trends-btn"> <span class="like-count" id="like-count-<?php echo $post['id']; ?>"><?php echo $post['like_count']; ?></span> Like(s)</button> <button class=" trends-btn"><?php echo $post['comment_count']; ?> Comment(s)</button>-->  <button class=" trends-btn">Trend Score: <?php echo $post['trending_score']; ?></button></small>
                    </div>
                    <br><div class="blog-content">
                    ...
                    <a href="view_post2.php?id=<?php echo $post['id']; ?>" class="animate-on-scroll btn">Read More</a>
                </li><br><br>
                <?php endforeach; ?>
            </ul>
        </div><br>
