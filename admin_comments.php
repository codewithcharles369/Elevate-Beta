...
$stmt = $pdo->query("SELECT comments.id, comments.comment, comments.post_id, posts.title AS post_title, users.name AS commenter 
                     FROM comments 
                     JOIN posts ON comments.post_id = posts.id 
                     JOIN users ON comments.user_id = users.id ORDER BY posts.created_at DESC");
...
<body class="<?php echo htmlspecialchars($theme); ?>">
    ...
    <aside class="sidebar" style="overflow-y: scroll;">
        ...
        <ul>
                ...
                <li><a href="admin_comments.php" class="active"><i class="fas fa-comments"></i>Manage Comments</a></li>
        ...
        <ul class="nav">
              ...
              <li  class="animate-on-scroll icon"><a href="admin_posts.php"><i class="fas fa-file-alt"></i></li>
            ...
            <input type="search" id="myInput" onkeyup="myFunction()" class="search" placeholder="Search for Comment.." title="Type in a Comment">
            ...
            <tbody>
                <?php foreach ($comments as $comment): ?>
                    <tr class="animate-on-scroll">
                        ...
                        <td><?php echo htmlspecialchars(substr($comment['post_title'], 0, 15)); ?>...</td>
                        <td><?php echo htmlspecialchars($comment['commenter']); ?></td>
                        <td>
                          <a class="btn " href="view_post2.php?id=<?php echo $comment['post_id']; ?>#<?php echo $comment['id']; ?>" ><i class="fas fa-eye"></i></a>
                            <a class="btn delete-btn" href="admin_delete_comment.php?id=<?php echo $comment['id']; ?>" onclick="return confirm('Delete this comment?');"><i class="fas fa-trash"></i></a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
